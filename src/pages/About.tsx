      ...
      <section className="bg-[#0A0F1C] py-24 md:py-32 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10 text-center max-w-4xl">
          <motion.h1 
            ...
            className="text-5xl md:text-7xl font-heading font-extrabold mb-8 tracking-tight"
          >
      ...
      <section className="py-24 md:py-32 bg-gray-50">
        <div className="container mx-auto px-4 md:px-8 max-w-7xl">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-heading font-bold mb-6 tracking-tight">Learning Outcomes</h2>
            <p className="text-gray-600 text-xl font-light max-w-2xl mx-auto">What students take away from participating in Skill Bazaar.</p>
          </div>
          
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {[
              { icon: Rocket, title: "Innovation", text: "Transforming raw ideas into marketable products." },
              { icon: Target, title: "Marketing", text: "Learning how to attract and retain customers effectively." },
              { icon: CheckCircle2, title: "Finance", text: "Understanding pricing, margins, and cash handling." },
              { icon: Flag, title: "Leadership", text: "Managing a team and taking ownership of a venture." },
            ].map((item, idx) => (
              <motion.div 
                key={idx}
                ...
                className="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 text-center hover:-translate-y-2 transition-transform duration-500"
              >
                <div className="w-20 h-20 mx-auto bg-gray-50 rounded-full flex items-center justify-center text-primary mb-8 border border-gray-100">
                  <item.icon size={32} />
                </div>
                <h3 className="text-2xl font-bold mb-4 font-heading">{item.title}</h3>
                <p className="text-gray-600 text-lg font-light leading-relaxed">{item.text}</p>
              </motion.div>
            ))}
          </div>
        </div>
